...
// 모델 클래스의 경우 이름의 끝에
// DTO 혹은 VO 가 붙는데
// 각각 Data Transfer Object 혹은 Value Object 의 의미를 가진다.
// DTO 가 좀더 넓은 의미를 가지므로 DTO 를 붙이면 왠만해서는 틀리지 않다.
// 모델 클래스에는 해당 데이터를 객체화 시킬때 필요한 필드, 겟터/셋터, equals()
// 정도의 메소드만 들어간다.

import lombok.Data;
// lombok 은 겟터/ 셋터, equals() 등 모델 클래스들에게 필요한.
// 기본 메소드를 자동으로 생성해주는 외부 라이브러리다.
// 필요에 따라서는 우리가 직접 @Getter / @Setter 등 필요한 것을
// 골라서 쓸 수도 있지만 @Data 라고 적어주면 모든 기능을 다 활성화시킨다.

@Data
// Data 가 게터/세터를 해주고 있기때문에 다시 할 필요가 없음
